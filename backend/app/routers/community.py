...
from app.models.community import Article, ArticleComment, LikeArticle
from app.models.accounts import User

from app.schemas.community import ArticleCreateForm, ArticleCommentForm, ArticleCommentList
...
@router.post("/notice-board", description="레시피 작성", response_model=CommonResponse, status_code=201)
async def create_notice(req: ArticleCreateForm, user: User = Depends(get_current_user)):
    # 유저 인증 로직
    await Article.create(user_id=user.id, category=0, **req.dict())
    return CommonResponse()


@router.post("/free-board", description="레시피 작성", response_model=CommonResponse, status_code=201)
async def create_free_article(req: ArticleCreateForm, user: User = Depends(get_current_user)):
    # 유저 인증 로직
    await Article.create(user_id=user.id, category=1, **req.dict())
    return CommonResponse()


@router.post("/gallery", description="레시피 작성", response_model=CommonResponse, status_code=201)
